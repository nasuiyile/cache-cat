...
const WRONG_VALUE_TYPE: &str = "WRONGTYPE Operation against a key holding the wrong kind of value";
...
impl PfAddParams {
    fn parse(items: &[Value]) -> Result<Self, ProtocolError> {
        ...
        let mut elements = Vec::with_capacity(items.len().saturating_sub(2));
...
impl RaftCommand for PfAddCommand {
    fn raft_request(&self, items: &[Value]) -> Result<Operation, ProtocolError> {
        let params = PfAddParams::parse(items)?;

        Ok(Operation::Base(PfAdd(PfAddReq {
            key: params.key,
    ...
    fn mutate(
        ...
        match &entry.value.data {
            ...
            ValueObject::String(raw) => {
                ...
                let mut hll = match RedisHll::decode(raw.as_ref()) {
                    Ok(hll) => hll,
                ...
                (
                    MochaOperation::Insert {
                        value: MyValue::new(ValueObject::String(bytes)),
