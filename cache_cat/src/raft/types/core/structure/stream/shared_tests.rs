...
fn grouped() -> SharedStream {
    ...
    core.xgroup_create(b"g", GroupStart::Id(StreamId::ZERO), None)
        .unwrap();
...
pub(super) fn with_contended_lock<T: Send>(
    held: impl Sized,
...
async fn timeout_includes_initial_lock_wait_for_both_read_modes() {
    ...
    assert!(
        s.xread_blocking(ReadStart::Tail, None, Block::For(Duration::from_millis(20)))
            .await
            .unwrap()
            .is_empty()
    );
    ...
    let started = Instant::now();
    assert!(
        s.xreadgroup_blocking(
            b"g",
            b"c",
            GroupRead::New,
            options(),
            Block::For(Duration::from_millis(20))
        )
        .await
        .unwrap()
        .is_empty()
...
async fn cancellation_of_notified_group_reader_hands_off() {
    let s = grouped();
    let mut a =
        Box::pin(s.xreadgroup_blocking(b"g", b"a", GroupRead::New, options(), Block::Forever));
    let mut b =
        Box::pin(s.xreadgroup_blocking(b"g", b"b", GroupRead::New, options(), Block::Forever));
    ...
    drop(a);
    drop(held);
    let entries = timeout_at(Instant::now() + Duration::from_secs(1), b)
        .await
        .unwrap()
        .unwrap();
...
async fn expired_notified_group_reader_hands_off_without_relocking() {
    let s = grouped();
    let mut a = Box::pin(s.xreadgroup_blocking(
        b"g",
        b"a",
        GroupRead::New,
        options(),
...
async fn blocking_readers_are_send_and_release_locks_while_waiting() {
    ...
    let id = {
        let s = s.clone();
        tokio::spawn(async move { s.xadd(AddId::Auto, data()) })
            .await
            .unwrap()
            .unwrap()
    };
...
async fn panicking_mutation_poisoning_wakes_blocked_readers() {
    ...
    let mut group =
        Box::pin(s.xreadgroup_blocking(b"g", b"c", GroupRead::New, options(), Block::Forever));
    ...
    assert!(matches!(
        s.xadd(AddId::Auto, data()),
        Err(StreamError::LockPoisoned)
    ));
}
    ...
    drop(f);
    {
        ...
        assert!(
            state
                .groups
                .values()
                .all(|signal| signal.upgrade().is_none())
        );
    }
    s.xadd(AddId::Auto, data()).unwrap();
