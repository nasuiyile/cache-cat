...
fn encode_snapshot_and_memory_wait_synchronously_for_write_lock() {
    ...
    let bytes = super::tests::with_contended_lock(s.0.state.write(), || {
        ...
        s.serialize_with(&mut serde_json::Serializer::new(&mut bytes))
            .unwrap();
        bytes
    });
    assert!(serde_json::from_slice::<SharedStream>(&bytes).is_ok());
    let snapshot = super::tests::with_contended_lock(s.0.state.write(), || s.snapshot().unwrap());
    assert!(snapshot.entries.is_empty());
    let memory = super::tests::with_contended_lock(s.0.state.write(), || s.memory_usage().unwrap());
...
async fn restore_has_fresh_identity_locks_and_empty_signal_registry() {
    ...
    assert!(!Arc::ptr_eq(
        &original.stream.group_identity(b"g").unwrap(),
...
async fn restored_group_readers_can_wait_and_receive_new_appends() {
    ...
    assert!(once(waiting.as_mut()).await.is_pending());
    let id = restored
        .xadd(AddId::Auto, vec![(b"f".to_vec(), b"v".to_vec())])
        .unwrap();
    let entries = tokio::time::timeout(Duration::from_secs(1), waiting)
        .await
        .unwrap()
        .unwrap();
